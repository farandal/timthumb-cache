 ...
 */
 ...
 /**
 * Modified version by farandal
 * http://www.github.com/farandal/timthumb
 */
 
//Gets server variables from nginx
//This script needs special nginx rules to work: https://gist.github.com/farandal/5923317

$cacheDir = $_SERVER['CACHE_DIR'];
$siteRoot = $_SERVER['SITE_ROOT'];
$domainName = $_SERVER['SERVER_NAME'];
$minifyRequest = $_SERVER['SCRIPT_NAME'];
$requesturi = $_SERVER['REQUEST_URI'];
$fragments = explode("/",$requesturi);
$size = explode("-",$fragments[2]);
...
$file = implode("/",$fragments); 

//http://www.yourdomain.com/thumb/{width}/{height}/{file}

//This part of the configuration was removed in my case, as I only needed to manage width and height from url.
...
if(! defined('DEBUG_LEVEL') )				define ('DEBUG_LEVEL', 100);								// Debug level 1 is less noisy and 3 is the most noisy
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', $cacheDir);				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
...
//			$this->cacheFile = $this->cacheDirectory . '/' .$this->src;


$pi = pathinfo($_SERVER['SITE_ROOT'].$_SERVER['REQUEST_URI']);
mkdir($pi['dirname'], 0777, TRUE);
$this->cachefile = $_SERVER['SITE_ROOT'].$_SERVER['SCRIPT_NAME'];
...
$this->cachefile = $_SERVER['SITE_ROOT'].$_SERVER['SCRIPT_NAME'];
	...
	protected function serveCacheFile(){
		...
		if(! is_file($this->cachefile)){
			$this->error("serveCacheFile called in timthumb but we couldn't find the cached file.".$this->cachefile);
			return false;
		}
